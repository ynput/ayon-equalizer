...
class ZipFileLongPaths(zipfile.ZipFile):
    ...
    def _extract_member(self, member: str, tpath: str, pwd: str) -> str:
        """Extract a member from the archive to the target path."""
        if self._is_windows:
...
def build_frontend() -> None:
    ...
    if yarn_executable is None:
        error_msg = "Yarn executable was not found."
        raise RuntimeError(error_msg)

    subprocess.run(
        [yarn_executable, "install"], cwd=FRONTEND_ROOT, check=False)
    subprocess.run(
        [yarn_executable, "build"], cwd=FRONTEND_ROOT, check=False)
    if not os.path.exists(FRONTEND_DIST_ROOT):
        error_msg = "Frontend build failed. Did not find 'dist' folder."
        raise RuntimeError(error_msg)
...
def get_client_zip_content(log: logging) -> io.BytesIO:
    """Prepare client code zip file."""
...
def get_base_files_mapping() -> list[FileMapping]:
    """Mapping of source server code files to destination paths."""
...
def create_addon_package(
    ...
    """Create package for addon."""
    log.info(
        "Creating package for %s-%s", ADDON_NAME, ADDON_VERSION)
    ...
    *,
    ...
    if only_client:
        if not has_client_code:
            error_msg = "Client code is not available. Skipping"
            raise RuntimeError(error_msg)
...
if __name__ == "__main__":
    ...
    main(args.output_dir,
         skip_zip=args.skip_zip, only_client=args.only_client)
